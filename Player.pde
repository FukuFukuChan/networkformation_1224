class Player {
  int index ;
  PlayerSet buildSet ; //辺を構築したプレーヤーの集合
  ...
  boolean hasBuildDeviation() { //逸脱として追加できる辺が存在するか
    ...
    for(Player p : cs) {
      ...
      if (cost < currentCost) {
        ...
        // println("selacted Player : " + p.index) ;
        // println("cost : " + cost + " < currentCost : " + currentCost) ;
        buildPlayer = p ;
        return true ;
      }
    }
    ...
    for(Player p : bSet) {
      ...
      for(Player pp : cs) {
        ...
        if (cost < currentCost) {
          // println("----------------------------------------------") ;
          // println("Player : " + index) ;
          // println("selacted Player : " + p.index + "," + pp.index) ;
          // println("cost : " + cost + " < currentCost : " + currentCost) ;
          buildPlayer = pp ;
          removePlayer = p ;
          return true ;
  ...
  void deviation() {
    if (buildPlayer != null) buildLink(buildPlayer) ;
    if (removePlayer != null) removeLink(removePlayer) ;
  }
}
